...
class MiddleFusionAttentionMobile(nn.Module):
    def __init__(self, num_classes, cue_dim=768, audio_width_mult=1.0):
        ...
        cue_dim = self.cue_encoder.output_dim
        fusion_dim = audio_dim + cue_dim
    ...
    def forward(self, mel, cue):
        audio_feat = self.audio_encoder(mel)    # (B, audio_dim)
        cue_feat = self.cue_encoder(cue)        # (B, cue_dim)

        fused = torch.cat([audio_feat, cue_feat], dim=1).unsqueeze(1)  # (B, 1, fusion_dim)
        attn_out, _ = self.cross_attn(fused, fused, fused)             # self-attention on fused features
