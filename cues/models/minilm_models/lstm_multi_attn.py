...
# ---------------------------------------------------------------------------
# Multi-head Self-Attention + LSTM Classifier
# ---------------------------------------------------------------------------
class MultiHeadSelfAttentionLSTM(nn.Module):
    def __init__(self, embed_dim, hidden_dim, num_labels, n_heads=4, dropout=0.3):
        super().__init__()
        self.lstm = nn.LSTM(
    ...
    def forward(self, x):
        ...
        attn_out = attn_out.mean(dim=1)  # aggregate across tokens
...
def train(model, train_loader, valid_loader, device, lr, epochs, metrics_path, model_path,
          class_weights, warmup_steps, model_name="minilm_lstm_transformer"):
...
def main():
    ...
    model = MultiHeadSelfAttentionLSTM(embed_dim=embed_dim, hidden_dim=128, num_labels=len(unique_words))
    model.to(device)
    ...
    train(model, train_loader, valid_loader, device, LR, EPOCHS, metrics_path, models_path,
          class_weights, warmup_steps, model_name="minilm_lstm_multihead_attn")
