...
def embed_sentences(model, sentences):
    return model.encode(sentences, convert_to_numpy=True, show_progress_bar=True)

# ---------------------------------------------------------------------------
# Dense Neural Network Classifier
# ---------------------------------------------------------------------------
class DenseClassifier(nn.Module):
    def __init__(self, input_dim, num_labels, hidden_dims=[512, 256, 256], dropout=0.3):
        super().__init__()
        layers = []
        prev_dim = input_dim
...
# Training Loop
# ---------------------------------------------------------------------------
def train(model, train_loader, valid_loader, device, lr, epochs, metrics_path, model_path, class_weights, model_name="dense_nn"):
...
def main():
    ...
    model = DenseClassifier(input_dim=input_dim, num_labels=len(unique_words))
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)

    train(model, train_loader, valid_loader, device, LR, EPOCHS, metrics_path, models_path, class_weights)
