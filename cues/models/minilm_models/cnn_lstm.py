...
def embed_sentences_tokenwise(tokenizer, model, sentences, device, max_length=32):
    """
    Returns token-level embeddings for each sentence.
    Output shape: (num_sentences, max_length, hidden_size)
    """
    model.to(device)
    all_embeddings = []
    ...
    for sent in tqdm(sentences, desc="Embedding sentences"):
        encoded = tokenizer(
            sent, truncation=True, padding="max_length", max_length=max_length,
            return_tensors="pt"
        )
        ...
        with torch.no_grad():
            ...
            token_embeds = outputs.last_hidden_state.squeeze(0)  # (seq_len, hidden)
...
class CNNLSTMClassifier(nn.Module):
    def __init__(self, embed_dim, hidden_dim, num_labels, kernel_sizes=[2,3,4], n_filters=64, dropout=0.3):
        super().__init__()
        # Convolution layers for different kernel sizes
        self.convs = nn.ModuleList([
            nn.Conv1d(in_channels=embed_dim, out_channels=n_filters, kernel_size=k)
            for k in kernel_sizes
        ])
        self.lstm = nn.LSTM(
            input_size=n_filters * len(kernel_sizes),
    ...
    def forward(self, x):
        ...
        conv_outs = [torch.relu(conv(x)) for conv in self.convs]  # each: (batch, n_filters, L_out)
        pooled_outs = [torch.max(out, dim=2)[0] for out in conv_outs]  # (batch, n_filters)
        cnn_out = torch.cat(pooled_outs, dim=1)  # (batch, n_filters * len(kernel_sizes))
        cnn_out = cnn_out.unsqueeze(1)  # (batch, 1, n_filters * len(kernel_sizes)) for LSTM
        lstm_out, _ = self.lstm(cnn_out)  # (batch, 1, hidden*2)
...
def train(model, train_loader, valid_loader, device, lr, epochs, metrics_path, model_path,
          class_weights, warmup_steps, model_name="minilm_cnn_lstm"):
...
def main():
    ...
    model = CNNLSTMClassifier(embed_dim=embed_dim, hidden_dim=128, num_labels=len(unique_words))
    model.to(device)
